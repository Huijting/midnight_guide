...
const HELP_CONTENT = {
  ...
  nl: {
    ...
    sections: [
      ...
      { icon:'📋', title:'/way coördinaten',  text:`Zie je een 📋 /way tekst staan? Tik erop om het te kopiëren en plak het in de WoW chat. Je hebt de TomTom addon nodig om de pijl te zien.\n\nIn de Midnight Guide-addon (in WoW): op Alle schatten/boeken rechtsklikken op een regel met /way zet een waypoint. Zit je in een delve of instance terwijl het waypoint op de open wereld staat, dan is de pijl vaak pas duidelijk buiten — de addon kan een gele tip in de chat tonen.` },
      ...
      { icon:'📋', title:'Changelog', text:`<strong>v1.0.6 (app)</strong> — 📡 <code>live_reset_data.json</code> (Wowhead EU Python-scraper) in CI naast prey/bountiful; Delves- en Prey-UI tonen scrape-tijd en Wowhead-Bountiful-namen; waarschuwing als TIW afwijkt van <code>bountiful-today.json</code>.<br><br><strong>v1.0.5 (app)</strong> — 🧭 Reisgids-tab, portaaldata (<code>data/portals.js</code>), footer- en PWA-versie gelijk (1.0.5). Zie <button type="button" class="about-open-inline" onclick="openAbout()">Over deze app</button>.<br><br><strong>v3.6.16 (app)</strong> — 🏰 Raids S1: ilvl 220/246/259/272 (+ apex 285), Alleria DPS-tips, Quel&apos;Danas 31 mrt 2026, Chiming Void Curio. Zie <button type="button" class="about-open-inline" onclick="openAbout()">Over deze app</button>.<br><br><strong>v3.6.15 (app)</strong> — 🎯 Prey: S1-beloningentabel uit JSON + 4 hunts/week Great Vault-tip. 💎 Delves: Tier 8 / Trovehunter ilvls mrt 2026. ⚙️ GitHub: workflow <code>fetch-eu-daily-json</code> — <code>prey-today.json</code> + <code>bountiful-today.json</code> tegelijk (zelfde retry-rooster als voorheen). Zie <button type="button" class="about-open-inline" onclick="openAbout()">Over deze app</button>.<br><br><strong>v3.6.14 (app)</strong> — 💎 Delves: EU Bountiful-dag dynamisch in de header; oude repo-json → weekrooster + waarschuwing. Mobiel: zichtbare sleutel naast sleutel-uitleg. Zie ook <button type="button" class="about-open-inline" onclick="openAbout()">Over deze app</button> voor de volledige lijst.<br><br><strong>v1.4</strong> — Handleiding met tabs + masterclass per rol. Rol-UI met visuele cues. Delves: Bountiful voor alle rollen + rol-suggesties waar van toepassing.<br><br><strong>v1.1</strong> — 💎 Delves: klik op een Delve-naam voor kort overzicht. 📖 Full Guide met events, boss-mechanieken en geheimen (NL/EN). Torment's Rise /way toegevoegd.<br><br><strong>v1.0</strong> — 🎯 Prey System toegevoegd met unlock questline. 📖 Guide & KP tabs samengevoegd. 👤 Karakter tracking voor KP schatten. 🎨 Nieuw 'Modern Dark Gaming' thema. 📱 Mobiele navigatie verbeterd. ⬆️ Back to top knop. 📋 Gekopieerd! notificaties.` },
    ],
    masterclass: {
      tank: {
        title: '🛡️ Tank',
  ...
  en: {
    ...
    sections: [
      ...
      { icon:'📋', title:'/way coordinates',   text:`See a 📋 /way text? Tap it to copy, then paste it in the WoW chat. You need the TomTom addon to see the arrow on screen.\n\nIn the Midnight Guide addon (in WoW): on All Treasures/Books, right-click a line with /way to set a waypoint. If you are in a delve or instance while the waypoint targets the outdoor map, the arrow is often clearest once you are outside — the addon can show a yellow tip in chat.` },
      { icon:'📱', title:'Install as app',     text:`Tap the install icon in your browser to save the guide as an app. It will work offline too!` },
      { icon:'📋', title:'Changelog', text:`<strong>v1.0.6 (app)</strong> — 📡 <code>live_reset_data.json</code> (Wowhead EU Python scraper) in CI with prey/bountiful; Delves + Prey UI show scrape time and Wowhead Bountiful names; warn if TIW disagrees with <code>bountiful-today.json</code>.<br><br><strong>v1.0.5 (app)</strong> — 🧭 Travel Guide tab, portal data (<code>data/portals.js</code>), footer and PWA versions aligned (1.0.5). See <button type="button" class="about-open-inline" onclick="openAbout()">About this app</button>.<br><br><strong>v3.6.16 (app)</strong> — 🏰 Raids S1: ilvl 220/246/259/272 (+ apex 285), Alleria DPS tips, Quel&apos;Danas Mar 31 2026, Chiming Void Curio. See <button type="button" class="about-open-inline" onclick="openAbout()">About this app</button>.<br><br><strong>v3.6.15 (app)</strong> — 🎯 Prey: Season 1 reward table from JSON + 4 hunts/week Great Vault tip. 💎 Delves: Tier 8 / Trovehunter ilvls Mar 2026. ⚙️ GitHub: <code>fetch-eu-daily-json</code> workflow — <code>prey-today.json</code> + <code>bountiful-today.json</code> in one run (same retry schedule as before). See <button type="button" class="about-open-inline" onclick="openAbout()">About this app</button>.<br><br><strong>v3.6.14 (app)</strong> — 💎 Delves: dynamic EU Bountiful day in the header; stale daily JSON falls back to the weekly rotation with a warning. Mobile: visible key next to key info. See <button type="button" class="about-open-inline" onclick="openAbout()">About this app</button> for the full changelog.<br><br><strong>v1.4</strong> — Help manual with tabs + per-role masterclass. Role UI with visual cues. Delves: Bountiful for all roles + role suggestions where applicable.<br><br><strong>v1.1</strong> — 💎 Delves: click a Delve name for short overview. 📖 Full Guide with events, boss mechanics and secrets (EN/NL). Torment's Rise /way added.<br><br><strong>v1.0</strong> — 📖 Glossary added (16 terms explained for beginners). Glossary tab + inline gold tooltips in dungeon tips. Nysarra corrected: STAND IN the wound = YOU deal 300% more damage TO the boss. Nav tabs horizontally scrollable.` },
    ],
    masterclass: {
      tank: {
        title: '🛡️ Tank',
