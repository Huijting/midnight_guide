...
const SPEC_FIRE_MAGE = {
  ...
  summary:{nl:"Fire Mage leeft voor Hot Streak procs. Twee crits op rij = gratis instant Pyroblast. Hoge crit-afhankelijkheid maar enorme burst wanneer het klikt.",en:"Fire Mage lives for Hot Streak procs. Two crits in a row = free instant Pyroblast. High crit dependency but massive burst when it clicks.",da:"Fire Mage lever for Hot Streak-procs. To crits i træk = gratis instant Pyroblast. Høj crit-afhængighed men massivt burst når det virker."},
  pros:{nl:["Enorme burst via Hot Streak procs", "Uitstekende AoE via Flamestrike", "Bevredigend om te spelen — proc-gebaseerd"],en:["Massive burst via Hot Streak procs", "Excellent AoE via Flamestrike", "Satisfying to play — proc-based"],da:["Massivt burst via Hot Streak-procs", "Fremragende AoE via Flamestrike", "Tilfredsstillende at spille — proc-baseret"]},
  cons:{nl:["RNG-afhankelijk — slechte procs = lage schade", "Hoge crit vereiste voor optimale prestaties", "Kwetsbaar bij movement"],en:["RNG-dependent — bad procs = low damage", "High crit requirement for optimal performance", "Vulnerable during movement"],da:["RNG-afhængig — dårlige procs = lav skade", "Høj crit-krav til optimal præstation", "Sårbar under bevægelse"]},
  cheatsheet:{
